...
function fetchWeatherByCity(city) {
    updateHighLowTemp(data); // Ensure high and low temperatures are updated
...
function updateHighLowTemp(data) {
    const highTempEl = document.getElementById("high-temp");
        lowTempEl.textContent = `Low: ${Math.round(data.main.temp_min || data.main.temp)} °C`;
    } else {
        console.error("Max/Min temperature not found in API response.");
        highTempEl.textContent = `High: -- °C`;
        lowTempEl.textContent = `Low: -- °C`;
    }
}
